  ...
  d3.spatialsankey = function () {
    ...
    spatialsankey.ranges = function () {
      ...
      nodes = nodes.map(function (node) {
        ...
        //Morteza: Make sure if target and source  are the same don't count them.
        var inflows = links.filter(function (link) { return (link.target == node.id && link.source !== node.id); });
        var outflows = links.filter(function (link) { return (link.source == node.id && link.target !== node.id); });
        //Morteza count the flow to itself
        var sameFlows = links.filter(function (link) { return (link.source == node.id && link.target == node.id); });
        ...
        //Morteza add the same flow
        node.properties.aggregate_sameFlow = sameFlows.reduce(function (memo, link) { return memo + link.flow; }, 0);

        return node;
      });

      // Calculate gobal ranges of values for links and nodes
      //Morteza: just consider inflows/outflows instead of all of links.
      ...
      var outflowsLinks = links.filter(function (link) { return (link.source !== link.target); });
      link_flow_range.min = d3.min(outflowsLinks, function (link) { return link.flow; });
      link_flow_range.max = d3.max(outflowsLinks, function (link) { return link.flow; });
      ...
      console.log("the max for outflows is:" + node_flow_range.max);
      console.log("the max for linkflow is:" + link_flow_range.max);
      console.log("the min for outflows is:" + node_flow_range.min);
      console.log("the min for linkflow is:" + link_flow_range.min);
    ...
    spatialsankey.link = function (options) {
      ...
      var link = function (d) {
        ...
        //Morteza: make sure links to the same node are not returned, otherwise there will be a huge circle around some nodes.
        if (dx == 0 && dy == 0) {
          return link;
        }
    ...
    spatialsankey.node = function (options) {
      ...
      node_color_range = ["yellow", "yellow"];
      ...
      node.r = function (d) {
        ...
        //Morteza  HACK just to adjust for America's extreme added multiplication * 15
        var r = (node_radius_range.max - node_radius_range.min) * (diff / range) * 15 + node_radius_range.min;
        //Morteza HACK This is done to cap circle sizes.the right way is to treat big outliers differently.
        if (r > 30) { r -= 30; }
        return r;
      };
      //Morteza added this for tooltip
      node.outFlows = function (d) {
        return d.properties.aggregate_outflows;
      }
      //Morteza for sameflows TODO you should get the flow_rang_min
      node.dd = function (d) {
        if (d.properties.aggregate_sameFlow == 0) {
          return 0;
        }
        var diff = d.properties.aggregate_sameFlow - node_flow_range.min,
          range = node_flow_range.max - node_flow_range.min;
        //Morteza  HACK just to adjust for America's extreme added multiplication * 15
        var dd = (node_radius_range.max - node_radius_range.min) * (diff / range) * 15 + node_radius_range.min;
        //Morteza This is done to cap circle sizes.the right way is to treat big outliers differently.
        if (dd > 30) { dd = dd - 180; }
        return dd;
      };
      //Morteza added this for tooltip
      node.sameFlows = function (d) {
        return d.properties.aggregate_sameFlow;
      }

      //Morteza added this for tooltip
      node.toolTip = function (d) {
        var countryName = spatialsankey.lookUpCountry(d.id)
        var str = countryName + "\n"
          + "People twitting from within " + countryName + " tweeted about \n"
          + d.properties.aggregate_outflows + " places in other countries and tweeted about\n"
          + d.properties.aggregate_sameFlow + " places within " + countryName + ".";
      ...
      node.fill = function (d) {
        var diff = d.properties.aggregate_outflows - node_flow_range.min,
          //Morteza: to account for america as an outlier.
          range = node_flow_range.max - node_flow_range.min - 200,
          load = diff / range;
        return color(load);
      };
      return node;
    ...
    spatialsankey.lookUpCountry = function (countryCode) {
